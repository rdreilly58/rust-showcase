...
Creates a simple task manager that adds tasks with priorities, filters them, serializes to JSON, and
demonstrates Rust's memory safety guarantees — all without a single runtime error possible.
...
- **Memory safety without GC** — same performance as C, but the compiler catches use-after-free,
